...
| Decrypted (unlocked) | `/dev/mapper/BackupSSD512GB` | `ext4` UUID | `/etc/fstab` |

### 🧠 The Golden Rule

> **`/etc/crypttab` → LUKS UUID**
> **`/etc/fstab` → ext4 (filesystem) UUID**
>
> Never mix them.

If you try to mount `/dev/nvme0n1p1` directly as ext4, you'll see:

```
EXT4-fs (nvme0n1p1): VFS: Can't find ext4 filesystem
```

That's because the partition is an encrypted container — the ext4 filesystem only exists *inside* the unlocked mapper device.
...
### 2. ext4 UUID (the unlocked/filesystem layer)
...
Look for `TYPE="ext4"`. Note the `UUID` value — this goes into `/etc/fstab`.
...
Add the following line (replace `XXXX-YYYY-ZZZZ` with your actual ext4 UUID):
...
> **Important:** Use the **ext4 UUID** here — not the LUKS UUID, and not `/dev/nvme0n1p1`.
...
- Mounting `/dev/nvme0n1p1` directly as ext4
- Putting the LUKS UUID in `/etc/fstab`
- Putting the ext4 UUID in `/etc/crypttab`
