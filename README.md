...
- Kernel hardening and system tuning via `sysctl` — expanded parameter set with explanations of what Linux Mint already sets vs what we override
- GRUB boot parameters — `slab_nomerge`, memory zeroing, page allocator shuffle, PTI, vsyscall hardening
- Swappiness tuning for better desktop responsiveness
- Cloudflare DNS over TLS with `systemd-resolved`
- System-wide ad and malware blocking via StevenBlack hosts file — auto-updated weekly with a systemd timer
...
- Remove unwanted packages (warpinator, Samba, Rhythmbox, Hypnotix, gnome-terminal, and more)
- Recommended software (Brave, KeePassXC, Goodvibes, Liferea, Kitty terminal, Tor Browser, Virt-Manager, and more)
- Kitty terminal setup — full config with Tokyo Night theme, tabs, splits, and JetBrains Mono
